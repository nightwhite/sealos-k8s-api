...
export const ingressController = new Elysia({ prefix: '/ingress' })
  ...
  /**
   * 获取 Ingress 详情（查询参数方式）
   * @description 通过查询参数获取指定 Ingress 的详细信息
   * @example
   * GET /api/v1/ingress/detail?name=my-ingress
   */
  .get('/detail', async ({ query, k8sService }) => {
    try {
      ...
      if (!name) {
        return {
          success: false,
          error: '请提供 Ingress 名称参数: ?name=ingress-name',
          example: `/api/v1/ingress/detail?name=my-ingress`,
          status: 400
        };
      }
